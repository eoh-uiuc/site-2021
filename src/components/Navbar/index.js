...
const Navbar = () => {
    ...
    return (
        <nav className={`navbar ${menuOpen ? 'navbar-open': 'navbar-close'} ${!menuOpen && !top ? 'scrolled': ''}`}>
            <Link onClick={hideMenu} to="/">
                <img className="navbar-logo" src={Logo} alt="logo" />
            </Link>

            <div className="menu-icon">
                <Hamburger animation="butterfly" toggle={toggleMenu} toggled={menuOpen}/>
            </div>
            ...
            <div className={`navbar-links ${menuOpen ? 'menu-links-open': 'menu-links-close'}`}>
                ...
                <Link className="navbar-text" onClick={hideMenu} to="/about">About Us</Link>
            </div>
        </nav>
    );
}
